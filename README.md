...
yarn add @seijs/redux-neuro

```

## Usage

To use it, implement *Slice* and it's *Neuron*  as follows.
```typescript
...
const loadUserBite = createNeuron<IUserTriggers, ITriggers, IUserState, IState, 'loadUser'>(
  {
  	wait:  loadUserWait,
	done:  loadUserDone,
	fail:  null,
...
const resetUserStateBite = createNeuron<IUserTriggers, ITriggers, IUserState, IState, 'resetUserState'>(
  resetUserState,
  null
);

export const userSlice = createSlice<IUserTriggers, ITriggers, IUserState, IState>(
  'user',
  {
...
```
where first argument of *createNeuron* function is reducer (or object of {pahaseName: phaseReduer}),
*loadUserWait* and *loadUserDone* behaves like they are mutating state.

```typescript
import { UserReducerType } from  '.';
import { MakeBiteReducerType } from '@seijs/redux-neuro/lib/types'
...
import { ScriptUpdateArgsType,
	 ScriptInitArgsType, 
	 ScriptOptsType } from  '@seijs/redux-neuro/lib/types';
...
export  class  LoadUser {
	...
	public  async  init(args: ScriptInitArgsType<IUserTriggers, 'loadUser', 'wait'>) {
		...
		** dispatch actions 
		** via this.opts.trigger('targetName', 'methodName',{...payload})
        ** get curent state via
        ** this.opts.getCurrentState()
		** wait for event to proceed via 
		** const payload =  await  this.opt.wait('targetName', 'methodName')
		*/ 
		setTimeout(() => {
	...
	public async  update(args: ScriptUpdateArgsType<IUserTriggers, 'loadUser', 'done'>) {
		...
		** call args.hold() to prevent reducer call.
		*/
